...
Note that most docker swarm deployments should be run as `mercury`. This is a user internal and specific to hgi team, and every member of the group can use its credentials to launch a service in an existing cluster, or to deploy/destroy a whole cluster.
...
`scp -i /path/to/mercury_private_key  /path/to/secret_file ubuntu@<Swarm Manager IP address>:~`
...
`scp -i /path/to/mercury_private_key  /path/to/docker-compose.yml ubuntu@<Swarm Manager IP address>:~`  

After that, we can either do Method 1 (ssh into remote and manually deploy) or Method 2 (execute an ansible task locally)

**Step 4 Method 1.**

ssh into the remote host as ubuntu using mercury's private key:

`ssh -i /path/to/mercury_private_key ubuntu@<Swarm Manager IP address>:~`  
...
`ansible all --key-file  /path/to/mercury_private_key  -i '<ubuntu@Swarm Manager IP address>,'-m docker_stack -a "name=app compose=docker-compose.yml" -vvv`
...
A swarm cluster uses an image based on the `docker-base` ansible role: 'theta-hgi-image-docker-base-0.1.2'. This image needs to exist on the tenant.
...
- Terraform returns with successful provisioning even if the bootup script is still running in the background (the bootup script that sets up the swarm does many tasks: in).  As a result, the swarm might not be up until after some time.
...
- DNS Issues:
  ...
  - For this reason, we reserve two dummy networks corresponding to 172.17.0.0/30 and 172.18.0.0/30 on each node at the time of provisioning, before the swarm initiation.
  - If any of the IP address from 172.17.0.0/30 and 172.18.0.0/30 or 172.19.0.0/16 are ever needed for an app, the provisioning tasks would need to unblock these and reserve something else for docker_gwbridge for the app to work.
...
- For some mysterious reason, as a default the user `ubuntu` is unable to talk to the docker daemon on the manager node, even if the user is manually added. For this reason, during provisioning, we loosen the permissions of the docker socket to be open to all users. (This may or may not be a security risk)
